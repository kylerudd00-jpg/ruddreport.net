...
const VESSEL_TYPES: Record<string, string> = {
  ...
  '54': 'Anti-Pollution', '55': 'Law Enforcement', '57': 'Spare', '58': 'Medical Transport',
  '59': 'Non-Combatant (RR)', '60': 'Passenger', '61': 'Passenger — Hazardous A',
  '62': 'Passenger — Hazardous B', '63': 'Passenger — Hazardous C',
  '64': 'Passenger — Hazardous D', '65': 'Passenger — Reserved',
  '66': 'Passenger — Reserved', '67': 'Passenger — Reserved',
  '68': 'Passenger — Reserved', '69': 'Passenger — No Additional Info',
      ...
      <div className="page-wrap">
        ...
        <div className="tool-section">
          ...
          <div className="result-panel">
                ...
                {results.map((v, i) => (
                  <div key={i} className="result-item" onClick={() => setSelected(v)}>
                    <div>
                      ...
                      <div className="result-meta">{vesselTypeLabel(v) || '—'} · Flag: {v.flag || '—'}</div>
                    </div>
            ...
            {selected && (
              <>
                ...
                <div className="detail-grid">
                  ...
                  {selected.length != null && <div className="detail-field"><div className="field-label">Length</div><div className="field-value">{selected.length} m</div></div>}
                  {selected.width != null && <div className="detail-field"><div className="field-label">Beam (Width)</div><div className="field-value">{selected.width} m</div></div>}
                  {selected.draught != null && <div className="detail-field"><div className="field-label">Draught</div><div className="field-value">{selected.draught} m</div></div>}
                  {selected.gross_tonnage != null && <div className="detail-field"><div className="field-label">Gross Tonnage</div><div className="field-value purple">{selected.gross_tonnage?.toLocaleString()} GT</div></div>}
                ...
                <div className="detail-grid">
                  ...
                  {selected.current_port && <div className="detail-field two"><div className="field-label">Current Port</div><div className="field-value blue">{selected.current_port}</div></div>}
                  {selected.destination && <div className="detail-field two"><div className="field-label">Destination</div><div className="field-value amber">{selected.destination}</div></div>}
            ...
            {!mmsiInfo && results === null && !noKey && !selected && (
              <div className="empty-panel">
                <div className="empty-icon">⚓</div>
                <div className="empty-text">Enter an MMSI to decode registration & flag state<br />or search by vessel name for full AIS details</div>
              </div>
            )}
          </div>
        </div>
